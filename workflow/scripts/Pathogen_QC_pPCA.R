...
# pPC <- pathogen_pca(path_pathogen='~/G2GSnake/raw_data/pathogen/AA_Table/OUT_aa_binary_table.txt',
#                     path_tree='~/G2GSnake/raw_data/pathogen/AA_Table/HBV_WG+og_rr.nw',n.pc=6)
# data.table::fwrite(pPC,'~/G2GSnake/results/tmp/pPC.txt',sep = ' ',col.names = T,row.names = F)

args <- commandArgs(trailingOnly = TRUE)
pPC <- pathogen_pca(path_pathogen=args[[1]],
                    path_tree=args[[2]],n.pc=as.numeric(args[[3]]),id.list = data.table::fread(args[[4]])$PID)
data.table::fwrite(pPC,args[[5]],sep = ' ',col.names = T,row.names = F)
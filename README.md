...
1. `host_VCF: "../raw_data/host/<File_Name>.vcf.gz"`
   - Path to VCF file
2. `covar_file: "../raw_data/<Covar_File_Name>.txt"`
   - Path to covars/mapping file
3. `host_MAF: <MAF_Threshold>`
   - Host variants below this threshold will be filtered out.
4. `host_missing: <Missing_Threshold>`
...
If Docker Desktop has been installed, ensure it has been started before running the pipeline or Shiny app.
...
<strong>IMPORTANT</strong>: From this step on, we will be working in the `workflow` directory: 
...
This method rely on docker and works on all platforms. 

A drawback is that the Snakefile cannot be modified to create custom rules or to modify existing rules.

Launch the G2GSnake pipeline which has been built as a Docker container. The number of cores can specified as an argument:

```
./run_snakemake_docker.sh <N_Cores>
```

### Method 2: With snakemake and singularity

#### Additional dependencies: Snakemake and Singularity
